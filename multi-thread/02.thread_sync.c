...
int main(){
    // 线程同步
    // 在多线程的编程过程中，对共享数据（多个线程共同访问的内存）做保护的。
    // 线程同步并不是让多个线程同时去做某一个事儿，而是让多个线程按照先后顺序依次执行。
    // 所谓的同步并不是多个线程同时对内存进行访问，而是按照先后顺序依次进行的。 这里带来的多余的时间开销就是为了保证数据的安全性
    ...
    // 并不能得到预期的结果，原因是因为，代码中两个线程共用全局变量number，每次要从内存中加载数据，然后进行运算，最后在将运算后的结果存放回内存中。
    // 如果在运算的中途，cpu时间片结束，会导致计算后的结果没有保存至内存中，所以另一个线程在加载数据的时候，加载的就是之前的结果。
    // 这样就导致了，多个线程访问共享资源出现数据混乱的问题。需要使用线程同步来解决
